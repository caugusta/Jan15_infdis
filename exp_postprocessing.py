...
def find_means(filename, reps, ignore_list=[]): #"Dec30_exponential_epidemics.txt"
	...
	if len(ignore_list) > 0:
        	for i in range(len(endtimes)): #original - worked for exponential case, but I ignored 0:199 there
                	if i % reps == 0:
                        	count_2 += 1
...
def epi_length(reps, pop):
	...
	if len(lines_seen) == len(parameter_sets):
		...
		exp_paras_means = exp_paras_and_means(tuple(parameter_sets), mean_epi_lengths)
	else:
...
def closest_paras(paras_and_means, exp_paras_means):
	...
	for i in range(len(paras_and_means)):
	     ...
	     for j in range(len(exp_paras_means)):
		  if test_set < set(exp_paras_means[j]) and j not in already_used:
...
def get_epidemics_assoc(reps, save_file = False):
	...
	#parameters_exponential_Jan14.txt has the parameter settings for each epidemic
	#Jan14_exponential_epidemics.txt has the epidemics (400 per parameter setting)
	...
	with open('exp_paras_listoflists.csv', 'r') as h:
	...
	for i in range(len(paras_kept)):
		...
		for j in range(len(para_list)):
			...
			if set1 == set2: #if the set of kept exp parameters (set1) is the same as the set of exp parameters we currently see
...
if __name__ == "__main__":
    ...
    c1 = closest_paras(paras_and_means, exp_para_means)
    ##print c1
    print np.asarray(c1)
