...
def epi_length(reps, pop):
	...
	if len(lines_seen) == len(parameter_sets):
		...
		neigh_paras_means = neigh_paras_and_means(tuple(parameter_sets), mean_epi_lengths)
	else:
...
def closest_paras(paras_and_means, neigh_paras_means):
	...
	for i in range(len(paras_and_means)):
	     ...
	     for j in range(len(neigh_paras_means)):
		  if test_set < set(neigh_paras_means[j]) and j not in already_used:
...
def get_epidemics_assoc(reps, save_file = False):
	...
	#parameters_neighbourhood_Jan14.txt has the parameter settings for each epidemic
	#Jan14_neighbourhood_epidemics.txt has the epidemics (400 per parameter setting)
	...
	with open('neigh_paras_listoflists.csv', 'r') as h:
	...
	print 'times', times[0:25]
	findones = find1(times) #there should be 6400 ones. These are the indices of the ones
	print 'length of findones', len(findones)
	...
	print 'original findones', findones
	print 'original endtimes', endtimes_indices

	for j in range(len(para_list)):
		para_list[j].pop(-1) #remove the empty string at the end of the list

	kept_epidemic = []
	for i in range(len(paras_kept)):
		paras_kept[i].pop(-1) #remove the mean epidemic length from the end of the list
		set1 = set([float(j) for j in paras_kept[i]])
		for j in range(len(para_list)):
			p1 = para_list[j][0].split(' ')
			set2 = set([float(k) for k in p1])
			if set1 == set2: #if the set of kept neigh parameters (set1) is the same as the set of neigh parameters we currently see
				#append the index of the success to a list
				kept_epidemic.append(j)

	print 'kept epidemics', kept_epidemic #these are the indices of the epidemics we want to keep
	...
	print 'findones to keep', findones
	print 'endtimes_indices to keep', endtimes_indices
	
	for k, l in zip(findones, endtimes_indices):
		epidemics_to_keep.append(epidata[k:(l+1),:]) #need the +1 because list[0:2] gives element 0 and element 1.

	print 'first epidemic to keep', epidemics_to_keep[0]
	...
	counter = 0
        ...
        for i in kept_epidemic:
                for j in range((i*pop), ((i+1)*pop)): #okat because goes to (i+1)*pop - 1, like 0, 199.
			counter += 1
			print counter
                        slice_keeper = np.concatenate([slice_keeper, [indivdata[j]]]) #([slice_keeper, indivdata[(i*300):((i*300)+199)]])
...
if __name__ == "__main__":
    ...
    c1 = closest_paras(paras_and_means, neigh_para_means)
    ##print c1
    print np.asarray(c1)
