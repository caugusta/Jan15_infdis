...
def pl_paras(pl_paras_means, ignore_these, reps, save_file=True):
	...
	#print 'pl_paras_means', len(pl_paras_means) #22
	#print 'len(para_list)', len(para_list) #12800
	kept_epidemic = []
	count = 0
	for i in range(len(pl_paras_means)):
		...
		for j in range(len(para_list)): #need to divide it into sections
			if j in ignore_these:
				continue
			else:
			#if j not in ignore_these:
	...
	print len(kept_epidemic) #should be 22*400 = 8800
