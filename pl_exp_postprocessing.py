#Just postprocessing for exp_epidemics.py, which was started on Jan14
...
def read_exp_indivs(pop): #Clean
        with open( 'epidemics_individual_exponential_Jan14.json', 'r' ) as f :
                    data = f.read()

        count = len(data.split('}, {'))
        #count = len(data.split('}{')) #1600 as desired
        print 'exponential epidemics indiv', count
        ...
        with open('Jan14_exponential_epidemics_individual.txt', 'w') as f:
...
def read_exp():
        ...
        with open( 'epidemics_exponential_Jan14.json', 'r' ) as f :
            data = f.read()
        ...
        with open('Jan14_exponential_epidemics.txt', 'w') as f:
                np.savetxt(f, np.c_[times, infindivs], fmt=['%i', '%i'])

        print 'wrote Jan14_exponential_epidemics.txt'
...
def exp_paras_and_means(paras, meanvec):
        ...
        with open('Jan14_exp_paras_and_means.csv', 'w') as newfile1:
...
def find_means(filename, reps, ignore_list=[]): #"Dec30_exponential_epidemics.txt"
        f = open(filename, 'r')
        data = f.readlines()
        times = []
        for x in data:
...
def epi_length(reps, pop):
	...
	#read in the mean epidemic length and the associated parameters from exponential
	with open('Jan14_pl_paras_and_means.csv', 'r') as tsv: #with open('parameters_and_means.txt', 'r') as tsv:
		...
		for row in reader1:
			...
			counter += 1
#	print 'exponential epidemics' #a list of lists

	read_exp() #already done
	read_exp_indivs(pop)

	#Calculate the mean epidemic length for the exponential epidemics
	...
	#Read in the parameters associated with all exponential epidemics
	for line in open('parameters_exponential_Jan14.txt', "r"):
		count_lines += 1
	...
	if len(lines_seen) == len(parameter_sets):
		#all is okay
		mean_epi_lengths = find_means("Jan14_exponential_epidemics.txt", reps)
		exp_paras_means = exp_paras_and_means(tuexpe(parameter_sets), mean_epi_lengths)
	else:
		...
		mean_epi_lengths = find_means("Jan14_exponential_epidemics.txt", reps, ignore_list = ignore_these)
		exp_paras_means = exp_paras_and_means(keep_para_list, mean_epi_lengths)
		closest_ones = closest_paras(paras_and_means, exp_paras_means)
		exp_paras_out = exp_paras(closest_ones, ignore_these, reps)
	print 'calculated mean epidemic lengths', mean_epi_lengths
	return (mean_epi_lengths, paras_and_means, exp_paras_means)
...
#For the exp and neigh versions, exp_paras and neigh_paras finds epidemics that are
#associated with parameters to be kept, but specifically related to the powerlaw epidemics.
#For example, say we find that parameter set [0.3, 2., 1., 0.01] for exponential gives the mean
...
#We do this for all parameter settings in exp_closest_to_pl_paras.
def exp_paras(exp_closest_to_pl_paras, ignore_these, reps, save_file=True): #, ignore_these, reps, save_file=True):

	#the parameters in exp_closest_to_pl_paras are not necessarily unique
	...
	#Open the parameter file for exponential
	para_list = []
	times = []
	with open('parameters_exponential_Jan14.txt', 'r') as f:
		for line in f:
			para_list.append(line.split('\n'))
	
	#Open the epidemic files for exponential
        with open('Jan14_exponential_epidemics.txt', 'r') as g:
                for line in g:
                        times.append(line.split(' ')[0])

        with open('Jan14_exponential_epidemics.txt', 'r') as g: #yes, I open it twice. Yes, I know this is bad.
                epidata = np.loadtxt(g)

        with open('Jan14_exponential_epidemics_individual.txt', 'r') as file1:
	...
	#print 'exp_paras_means', len(exp_closest_to_pl_paras) #number of unique sets of parameters (22)
	#print 'len(para_list)', len(para_list) #number of epidemics, total. (12800)

	#Find the epidemics associated with the kept parameters
	kept_epidemic = []
	count = 0
	for i in range(len(exp_closest_to_pl_paras)): #for exp and neigh, len should be 22
		exp_closest_to_pl_paras[i].pop(-1) #remove last element which is mean
		set1 = set([float(j) for j in exp_closest_to_pl_paras[i]])
	...
	#For the epidemics that are being kept
        ...
        if save_file:
                with open('Jan14_exponential_postprocessed.txt', 'w') as f:
                        np.savetxt(f, np.c_[etk_times, etk_infindivs], fmt=['%i', '%i'])

                with open('Jan14_exponential_individual_postprocessed.txt', 'w') as f:
                        np.savetxt(f, slice_keeper, fmt = ['%i', '%i'])			

	print 'wrote files'

def closest_paras(paras_and_means, exp_paras_means):
	exp_mean = []
	keep_paras = []
	index_list = []
	already_used = []
	for i in range(len(paras_and_means)):
	     ...
	     for j in range(len(exp_paras_means)):
		  if test_set < set(exp_paras_means[j]) and j not in already_used: #was < for other types of epidemic
		       exp_mean.append(exp_paras_means[j][-1])
		       index_list.append(j)
		  else:
		       continue
	     ar1 = (np.array(float(paras_and_means[i][0][-1])) - exp_mean)**2 #was float(paras_and_means[i][-1]
	     ind1 = np.where(ar1 == ar1.min())
             get_item = index_list[ind1[0][0]]
	     already_used.append(get_item)
	     keep_paras.append(exp_paras_means[get_item])


	with open('exp_paras_listoflists.csv', 'w') as f:
...
if __name__ == "__main__":
    reps = 400 #20 #400
    pop = 150 #300
    #r1 = read_exp_indivs(pop) #already called in epi_length below
    #r1 = read_exp()
    mean_exp_epi_lengths, paras_and_means, exp_para_means = epi_length(reps, pop)
    #c1 = closest_paras(paras_and_means, exp_para_means) #called from epi_length
    ##print c1
    #print np.asarray(c1)
    #g1 = get_epidemics_assoc(reps, save_file=True)
    #p2 = exp_paras(exp_para_means)
