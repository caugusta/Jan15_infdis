#Just postprocessing for neigh_epidemics.py, which was started on Jan14
...
#Purpose: read through the individual times to infection, parse in an R-readable format.
#Each epidemic here has pop (150) elements.
#{"1":6, "2":2, ..., "150":3} is an epidemic. 
#Want individuals as one column, times of infection in another

def read_neigh_indivs(pop): #Clean
        with open( 'epidemics_individual_neighbourhood_Jan14.json', 'r' ) as f :
                    data = f.read()

        count = len(data.split('}, {'))
        #count = len(data.split('}{')) #1600 as desired
        print 'neighbourhood epidemics indiv', count
        ...
        new1 = infindivs
        new1.append(str(str_epi2[0][(lastone+2):])) ## lastone: is ': number'. Don't want the colon or the space
        ...
        while i < len(str_epi2):
                ...
                lastone = str_epi2[0].rfind(':') #last occurrance of :
                newinf = infindivs_1
                newinf.append(str(str_epi2[0][(lastone+2):])) ## lastone: is ': number'. Don't want the colon or the space
                if len(newinf) < 2:
        ...
        with open('Jan14_neighbourhood_epidemics_individual.txt', 'w') as f:
...
#                    data = f.read()
#
#        count = len(data.split('}, {')) #number of epidemics
#        print 'neighbourhood epidemics indiv', count
#
#        str_epi = ' '.join(data.split('{'))
#        str_epi2 = str_epi.split('}')
...
def read_neigh():
        ...
        with open( 'epidemics_neighbourhood_Jan14.json', 'r' ) as f :
            data = f.read()
        ...
        with open('Jan14_neighbourhood_epidemics.txt', 'w') as f:
                np.savetxt(f, np.c_[times, infindivs], fmt=['%i', '%i'])

        print 'wrote Jan14_neighbourhood_epidemics.txt'
...
def neigh_paras_and_means(paras, meanvec):
        ...
        with open('Jan14_neigh_paras_and_means.csv', 'w') as newfile1:
...
def find_means(filename, reps, ignore_list=[]): #"Dec30_neighbourhood_epidemics.txt"
        f = open(filename, 'r')
        data = f.readlines()
        times = []
        for x in data:
...
def epi_length(reps, pop):
	...
	#read in the mean epidemic length and the associated parameters from neighbourhood
	with open('Jan14_pl_paras_and_means.csv', 'r') as tsv: #with open('parameters_and_means.txt', 'r') as tsv:
		...
		for row in reader1:
			...
			counter += 1
#	print 'neighbourhood epidemics' #a list of lists

	#read_neigh() #already done
	#read_neigh_indivs(pop)

	#Calculate the mean epidemic length for the neighbourhood epidemics
	...
	#Read in the parameters associated with all neighbourhood epidemics
	for line in open('parameters_neighbourhood_Jan14.txt', "r"):
		count_lines += 1
	...
	if len(lines_seen) == len(parameter_sets):
		#all is okay
		mean_epi_lengths = find_means("Jan14_neighbourhood_epidemics.txt", reps)
		neigh_paras_means = neigh_paras_and_means(tuneighe(parameter_sets), mean_epi_lengths)
	else:
		...
		mean_epi_lengths = find_means("Jan14_neighbourhood_epidemics.txt", reps, ignore_list = ignore_these)
		neigh_paras_means = neigh_paras_and_means(keep_para_list, mean_epi_lengths)
		closest_ones = closest_paras(paras_and_means, neigh_paras_means)
		neigh_paras_out = neigh_paras(closest_ones, ignore_these, reps)
	print 'calculated mean epidemic lengths', mean_epi_lengths
	return (mean_epi_lengths, paras_and_means, neigh_paras_means)

#Purpose: in the powerlaw version, pl_paras finds epidemics that are associated
#with parameter sets to be kept - unique parameter sets. 
#For example, if we want to keep epidemic 1, then we want to keep epidemics 0:(reps-1)
#for both the basic epidemic dataset (time:num inf) and the individual epidemic dataset (indiv: inftime)

#For the neigh and neigh versions, neigh_paras and neigh_paras finds epidemics that are
#associated with parameters to be kept, but specifically related to the powerlaw epidemics.
#For example, say we find that parameter set [0.3, 2., 1., 0.01] for neighbourhood gives the mean
#epidemic length closest to set [0.3, 1., 1., 0.01] in powerlaw. Then we want to get 
#all of the epidemics belonging to set [0.3, 2., 1., 0.01] and save them separately.
#We do this for all parameter settings in neigh_closest_to_pl_paras.
def neigh_paras(neigh_closest_to_pl_paras, ignore_these, reps, save_file=True): #, ignore_these, reps, save_file=True):

	#the parameters in neigh_closest_to_pl_paras are not necessarily unique
	#make sure to ignore the duplicates, as we did when finding the parameter sets that resulted in
	#mean epidemic lengths that were close to those in powerlaw
	ignore_these = set([int(j) for j in ignore_these])
	newignore = []
	for i in ignore_these:
		newignore.append(list(np.arange(i*reps, (i+1)*reps)))
	newignore = sum(newignore, []) #collapses the list of lists
	ignore_these = set(newignore)

	#Open the parameter file for neighbourhood
	para_list = []
	times = []
	with open('parameters_neighbourhood_Jan14.txt', 'r') as f:
		for line in f:
			para_list.append(line.split('\n'))
	
	#Open the epidemic files for neighbourhood
        with open('Jan14_neighbourhood_epidemics.txt', 'r') as g:
                for line in g:
                        times.append(line.split(' ')[0])

        with open('Jan14_neighbourhood_epidemics.txt', 'r') as g: #yes, I open it twice. Yes, I know this is bad.
                epidata = np.loadtxt(g)

        with open('Jan14_neighbourhood_epidemics_individual.txt', 'r') as file1:
                indivdata = np.loadtxt(file1)

        print 'finding the ones'
	#Numbers here reflect powerlaw epidemics (12800, etc).
	#Note there are only 8800 powerlaw epidemics kept, which should be the same for these.
        findones = find1(times) #there should be 12800 ones. These are the indices of the ones
	...
	#print 'neigh_paras_means', len(neigh_closest_to_pl_paras) #number of unique sets of parameters (22)
	#print 'len(para_list)', len(para_list) #number of epidemics, total. (12800)

	#Find the epidemics associated with the kept parameters
	kept_epidemic = []
	count = 0
	for i in range(len(neigh_closest_to_pl_paras)): #for neigh and neigh, len should be 22
		neigh_closest_to_pl_paras[i].pop(-1) #remove last element which is mean
		set1 = set([float(j) for j in neigh_closest_to_pl_paras[i]])
		for j in range(len(para_list)): 
			if j in ignore_these:
				continue
			else:
	...
	print len(kept_epidemic) #should be (#kept paras)*(reps) (22*400 = 8800)
        
	#Find the start and end of each epidemic we want to keep
        findones = [findones[i] for i in kept_epidemic]
        endtimes_indices = [endtimes_indices[i] for i in kept_epidemic]

	epidemics_to_keep = []

	#Append the epidemics to keep to a list
        for k, l in zip(findones, endtimes_indices):
                epidemics_to_keep.append(epidata[k:(l+1),:]) #was l...
	
	#Now for the individual times to infection
	#We have pop (150) individuals, so each 'epidemic' here has length pop.
	#Suppose we want to keep epidemic 5. That means we want indices
	#5*150 = 600 to (6*150)-1 = 749, because that's the fifth epidemic.
	#So if we have 8800 kept epidemics, we should have 8800*150 = 60,000 
	#elements in slice_keeper 
        ...
        slice_keeper = slice_keeper[2:] #only had first two elements so np.concatenate would work
        ...
        etk_indiv = []
        etk_indivtime = []
	#For the epidemics that are being kept
        for i in range(len(epidemics_to_keep)):
                for j in range(len(epidemics_to_keep[i])):
                        etk_times.append(epidemics_to_keep[i][j][0]) #get the times
                        etk_infindivs.append(epidemics_to_keep[i][j][1]) #and the num of inf indivs

        print 'finished etk'

        if save_file:
                with open('Jan14_neighbourhood_postprocessed.txt', 'w') as f:
                        np.savetxt(f, np.c_[etk_times, etk_infindivs], fmt=['%i', '%i'])

                with open('Jan14_neighbourhood_individual_postprocessed.txt', 'w') as f:
                        np.savetxt(f, slice_keeper, fmt = ['%i', '%i'])			

	print 'wrote files'

def closest_paras(paras_and_means, neigh_paras_means):
	neigh_mean = []
	keep_paras = []
	index_list = []
	already_used = []
	for i in range(len(paras_and_means)):
	     ...
	     for j in range(len(neigh_paras_means)):
		  if test_set < set(neigh_paras_means[j]) and j not in already_used: #was < for other types of epidemic
		       neigh_mean.append(neigh_paras_means[j][-1])
		       index_list.append(j)
		  else:
		       continue
	     ar1 = (np.array(float(paras_and_means[i][0][-1])) - neigh_mean)**2 #was float(paras_and_means[i][-1]
	     ind1 = np.where(ar1 == ar1.min())
             get_item = index_list[ind1[0][0]]
	     already_used.append(get_item)
	     keep_paras.append(neigh_paras_means[get_item])


	with open('neigh_paras_listoflists.csv', 'w') as f:
...
if __name__ == "__main__":
    reps = 400 #20 #400
    pop = 150 #300
    #r1 = read_neigh_indivs(pop) #already called in epi_length below
    #r1 = read_neigh()
    mean_neigh_epi_lengths, paras_and_means, neigh_para_means = epi_length(reps, pop)
    #c1 = closest_paras(paras_and_means, neigh_para_means) #called from epi_length
    ##print c1
    #print np.asarray(c1)
    #g1 = get_epidemics_assoc(reps, save_file=True)
    #p2 = neigh_paras(neigh_para_means)
